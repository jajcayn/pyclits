...
def right_side_ivp(t, y, params):
	x1, x2, x3, y1, y2, y3 = y  # unpack current values of y
	a1, a2, b1, b2, c1, c2, omega1, omega2, epsilon = params  # unpack parameters
	derivs = [-omega1 * x2 - x3, omega1 * x1 + a1 * x2, b1 + x3 * (x1 - c1), - omega2 * y2 - y3 + epsilon * (x1 - y1), omega2 * y1 + a2 * y2,
	          b2 + y3 * (y1 - c2)]
...
if __name__ == "__main__":
	...
	if (dataset):
		...
		for epsilon in np.arange(0.25, 0.55, 0.05):  # np.arange(0.0, 0.241, 0.005)
			...
			for fragment in range(fragments):
				...
				roessler_3d_plot(solution, configuration, fr"\Large Evolution of Rössler oscilator $\varepsilon= {epsilon}$", f"roessler-{epsilon}_{fragment}",
				                 "png")
