...
class DataField:
    ...
    def __init__(
        ...
        self.missing = (
            None  # for station data where could be some missing values
        )
    ...
    def __str__(self):
        ...
        if self.data is not None:
            return "Geo data of shape %s as time x lat x lon." % str(
                self.data.shape
            )
        else:
    ...
    def load(
        ...
        if dataset == "ECA-reanalysis":
            ...
            if print_prog:
                print(
                    "Data saved to structure. Shape of the data is %s"
                    % (str(self.data.shape))
                )
                print(
                    "Lats x lons saved to structure. Shape is %s x %s"
                    % (str(self.lats.shape[0]), str(self.lons.shape[0]))
                )
                print(
                    "Time stamp saved to structure as ordinal values where Jan 1 of year 1 is 1"
                )
                print(
                    "The first data value is from %s and the last is from %s"
                    % (
                        str(self.get_date_from_ndx(0)),
                        str(self.get_date_from_ndx(-1)),
                    )
                )
                print(
                    "Default temporal sampling in the data is %.2f day(s)"
                    % (np.nanmean(np.diff(self.time)))
                )
                if np.any(np.isnan(self.data)):
                    print(
                        "The data contains NaNs! All methods are compatible with NaNs, just to let you know!"
                    )
            ...
            if "hours" in d.variables["time"].units:
                ...
                for t in range(self.time.shape[0]):
                    self.time[t] = date.toordinal(
                        date_since + relativedelta(months=+int(self.time[t]))
                    )
            self.var_name = variable_name
            if np.any(np.isnan(self.data)):
                self.nans = True
            ...
            for key in d.variables.keys():
                ...
                try:  # handling when some netCDF variable hasn't assigned units
                    if "since" in d.variables[key].units:
                        ...
                        date_since = self._parse_time_units(
                            d.variables[key].units
                        )
                        if "hours" in d.variables[key].units:
                            self.time = self.time / 24.0 + date.toordinal(
                                date_since
                            )
                        elif "seconds" in d.variables[key].units:
                            self.time = self.time / 86400.0 + date.toordinal(
                                date_since
                            )
                            ...
                            for t in range(self.time.shape[0]):
                                self.time[t] = date.toordinal(
                                    date_since
                                    + relativedelta(months=+int(self.time[t]))
                                )
    ...
    def load_station_data(
        self, filename, dataset="ECA-station", print_prog=True, offset_in_file=0
    ):
        """
        ...
        if dataset == "Klem_day":
            ...
            )  # first column is continous year and second is actual data
            ...
            print(
                "Station data from %s saved to structure. Shape of the data is %s"
                % (self.location, str(self.data.shape))
            )
            print(
                "Time stamp saved to structure as ordinal values where Jan 1 of year 1 is 1"
            )
        ...
        if dataset == "ECA-station":
            with open(self.data_folder + filename, "rb") as f:
                ...
                for row in reader:
                    ...
                    if (
                        i > 20 + offset_in_file
                    ):  # actual data - len(row) = 5 as STAID, SOUID, DATE, TG, Q_TG
                        staid = int(row[0])
                        value = float(row[3])
                        year = int(row[2][:4])
                        month = int(row[2][4:6])
            ...
            if self.missing.shape[0] != 0 and self.verbose:
                print(
                    "** WARNING: There were some missing values! To be precise, %d missing values were found!"
    ...
    def select_date(
        self, date_from, date_to, apply_to_data=True, exclusive=True
    ):
        """
        ...
        if apply_to_data:
            ...
            if self.data_mask is not None and self.data_mask.ndim > 2:
                self.data_mask = self.data_mask[
                    ndx, ...
                ]  # slice missing if exists
        if self.missing is not None:
            missing_ndx = np.logical_and(
                self.missing >= d_start, self.missing < d_to
            )
            self.missing = self.missing[missing_ndx]  # slice missing if exists

        return ndx

    def get_sliding_window_indexes(
        self, window_length, window_shift, unit="m", return_half_dates=False
    ):
        """
        ...
        if unit == "m":
            ...
            raise Exception(
                "Unknown time unit! Please, use one of the 'd', 'm', 'y'!"
            )
        ...
        while window_end <= self.get_date_from_ndx(-1):
            ndx = self.select_date(
                window_start, window_end, apply_to_data=False
            )
            ndxs.append(ndx)
            if return_half_dates:
                half_dates.append(
        ...
        # add last
        ndxs.append(
            self.select_date(window_start, window_end, apply_to_data=False)
        )
        if return_half_dates:
            half_dates.append(
                window_start + (self.get_date_from_ndx(-1) - window_start) / 2
            )

        if np.sum(ndxs[-1]) != np.sum(ndxs[-2]) and self.verbose:
            print(
                "**WARNING: last sliding window is shorter than others! (%d vs. %d in others)"
    ...
    def get_closest_lat_lon(self, lat, lon):
        ...
        return [
            np.abs(self.lats - lat).argmin(),
            np.abs(self.lons - lon).argmin(),
        ]
    ...
    def select_months(self, months, apply_to_data=True):
        ...
        ndx = filter(
            lambda i: date.fromordinal(int(self.time[i])).month in months,
            range(len(self.time)),
        )
    ...
    def select_lat_lon(self, lats, lons, apply_to_data=True):
        ...
        if self.lats is not None and self.lons is not None:
            if lats is not None:
                lat_ndx = np.nonzero(
                    np.logical_and(self.lats >= lats[0], self.lats <= lats[1])
                )[0]
            else:
            ...
            if lons is not None:
                if lons[0] < lons[1]:
                    lon_ndx = np.nonzero(
                        np.logical_and(
                            self.lons >= lons[0], self.lons <= lons[1]
                        )
                    )[0]
                elif lons[0] > lons[1]:
                    l1 = list(
                        np.nonzero(
                            np.logical_and(
                                self.lons >= lons[0], self.lons <= 360
                            )
                        )[0]
                    )
                    l2 = list(
                        np.nonzero(
                            np.logical_and(self.lons >= 0, self.lons <= lons[1])
                        )[0]
                    )
            ...
            raise Exception(
                "Slicing data with no spatial dimensions, probably station data."
            )
    ...
    def cut_lat_lon(self, lats_to_cut, lons_to_cut):
        ...
        if self.lats is not None and self.lons is not None:
            if lats_to_cut is not None:
                lat_ndx = np.nonzero(
                    np.logical_and(
                        self.lats >= lats_to_cut[0], self.lats <= lats_to_cut[1]
                    )
                )[0]
            ...
            if lons_to_cut is not None:
                if lons_to_cut[0] < lons_to_cut[1]:
                    lon_ndx = np.nonzero(
                        np.logical_and(
                            self.lons >= lons_to_cut[0],
                            self.lons <= lons_to_cut[1],
                        )
                    )[0]
                    ...
                    l1 = list(
                        np.nonzero(
                            np.logical_and(
                                self.lons >= lons_to_cut[0], self.lons <= 360
                            )
                        )[0]
                    )
                    l2 = list(
                        np.nonzero(
                            np.logical_and(
                                self.lons >= 0, self.lons <= lons_to_cut[1]
                            )
                        )[0]
                    )
                    lon_ndx = np.array(l1 + l2)
    ...
    def latitude_cos_weights(self):
        ...
        if (np.all(self.cos_weights) is not None) and (
            self.cos_weights.shape == self.get_spatial_dims()
        ):
        ...
        if f is None:
            if self.data.ndim == 3:
                self.data = np.reshape(
                    self.data,
                    (self.data.shape[0], np.prod(self.data.shape[1:])),
                )
            else:
                raise Exception(
                    "Data field is already flattened, multi-level or only temporal (e.g. station)!"
                )
            ...
            if f.ndim == 3:
                ...
                raise Exception(
                    "The field f is already flattened, multi-level or only temporal (e.g. station)!"
                )
    ...
    def reshape_flat_field(self, f=None):
        ...
        if f is None:
            if self.data.ndim == 2:
                new_shape = [self.data.shape[0]] + list(
                    (self.lats.shape[0], self.lons.shape[0])
                )
                self.data = np.reshape(self.data, new_shape)
            else:
                raise Exception(
                    "Data field is not flattened, is multi-level or is only temporal (e.g. station)!"
                )

        elif f is not None:
            if f.ndim == 2:
                new_shape = [f.shape[0]] + list(
                    (self.lats.shape[0], self.lons.shape[0])
                )
                ...
                raise Exception(
                    "The field f is not flattened, is multi-level or is only temporal (e.g. station)!"
                )

    def get_data_of_precise_length(
        self, length="16k", start_date=None, end_date=None, apply_to_data=False
    ):
        """
        ...
        if isinstance(length, int):
            ...
            raise Exception(
                'Could not understand the length! Please type length as integer or as string like "16k".'
            )
        ...
        if end_date is None and start_date is not None:
            ...
            raise Exception(
                "You messed start / end date selection! Pick only one!"
            )
    ...
    def get_annual_data(self, means=True, ts=None):
        ...
        for y in range(year[0], year[-1] + 1, 1):
            ...
            if ts is None:
                if means:
                    yearly_data.append(
                        np.squeeze(np.nanmean(self.data[year_ndx, ...], axis=0))
                    )
                else:
                    yearly_data.append(
                        np.squeeze(np.nansum(self.data[year_ndx, ...], axis=0))
                    )
            else:
                if means:
                    yearly_data.append(
                        np.squeeze(np.nanmean(ts[year_ndx, ...], axis=0))
                    )
                else:
                    yearly_data.append(
                        np.squeeze(np.nansum(ts[year_ndx, ...], axis=0))
                    )
    ...
    def get_monthly_data(self, means=True):
        ...
        if delta == 1:
            ...
            while end_idx <= self.data.shape[0] and end_idx is not None:
                if means:
                    monthly_data.append(
                        np.nanmean(self.data[start_idx:end_idx, ...], axis=0)
                    )
                else:
                    monthly_data.append(
                        np.nansum(self.data[start_idx:end_idx, ...], axis=0)
                    )
                ...
                if end_idx is None:  # last piece, then exit the loop
                    if means:
                        monthly_data.append(
                            np.nanmean(self.data[start_idx:, ...], axis=0)
                        )
                    else:
                        monthly_data.append(
                            np.nansum(self.data[start_idx:, ...], axis=0)
                        )
                    monthly_time.append(self.time[start_idx])
    ...
    def average_to_daily(self):
        ...
        if delta < 1:
            ...
            d = np.delete(
                d, slice(0, (n_times - 1) * d.shape[0] / n_times), axis=0
            )
            t = np.zeros(self.time.shape[0] / n_times)
            for i in range(d.shape[0]):
                d[i, ...] = np.nanmean(
                    self.data[n_times * i : n_times * i + (n_times - 1), ...],
                    axis=0,
                )
            ...
            raise Exception(
                "No sub-daily values, you can average to daily only values with finer time sampling."
            )
    ...
    def interpolate_to_finer_temporal_resolution(
        self, to_resolution="m", kind="linear", use_to_data=False, pool=None
    ):
        """
        ...
        job_args = [
            (i, j, self.time, self.data[:, i, j], new_time, kind)
            for i in range(num_lats)
            for j in range(num_lons)
        ]

        interp_data = np.zeros(
            [new_time.shape[0]] + list(self.get_spatial_dims())
        )
    ...
    def _ascending_descending_lat_lons(
        self, lats=True, lons=False, direction="asc"
    ):
        """
    ...
    def subsample_spatial(self, lat_to, lon_to, start, average=False):
        ...
        if self.lats is not None and self.lons is not None:
            ...
            if lat_to % delta_lats == 0 and lon_to % delta_lons == 0:
                ...
                lat_flg, lon_flg = self._ascending_descending_lat_lons(
                    lats=True, lons=True, direction="asc"
                )
                ...
                if start_lon_ndx.size == 1 and start_lat_ndx.size == 1:
                    ...
                    if not average:
                        ...
                        if self.nans:
                            if self.check_NaNs_only_spatial():
                                ...
                                raise Exception(
                                    "NaNs in the data are not only spatial, cannot interpolate!"
                                )
                        ...
                        new_lats = np.arange(
                            start[0], self.lats[-1] + lat_to, lat_to
                        )
                        new_lons = np.arange(start[1], self.lons[-1], lon_to)
                        d = np.zeros(
                            (
                                list(self.data.shape[:2])
                                + [new_lats.shape[0], new_lons.shape[0]]
                            )
                        )
                        # interpolate using Bivariate spline
                        for t in range(self.time.shape[0]):
                            for lvl in range(self.data.shape[1]):
                                int_scheme = RectBivariateSpline(
                                    self.lats, self.lons, self.data[t, lvl, ...]
                                )
                    ...
                    raise Exception(
                        "Start lat and / or lon for subsampling does not exist in the data!"
                    )

                self._ascending_descending_lat_lons(
                    lats=lat_flg, lons=lon_flg, direction="des"
                )

            else:
                raise Exception(
                    "Subsampling lats only to multiples of %.2f and lons of %.2f"
                    % (delta_lats, delta_lons)
                )

        else:
            raise Exception(
                "Cannot subsample station data, or data from one grid point!"
            )

    def smoothing_running_avg(
        self, points, cut_edges=False, use_to_data=False, ts=None
    ):
        """
        ...
        for i in range(d.shape[0]):
            if cut_edges:
                ...
                d[i, ...] = np.nanmean(
                    ts[max(i - window, 1) : min(i + window, d.shape[0]), ...],
                    axis=0,
                )
    ...
    def plot_FFT_spectrum(
        self, ts=None, log=True, vlines=np.arange(1, 11), fname=None
    ):
        """
        ...
        if log:
            plt.semilogx(
                freqs, 20 * np.log10(fft), linewidth=0.8
            )  # in dB hopefully...
        ...
        for vline in vlines:
            plt.axvline(
                1.0 / vline, 0, 1, linestyle=":", linewidth=0.6, color="#333333"
            )
    ...
    def temporal_filter(
        ...
        if type(cutoff) == list and btype in ["bandpass", "bandstop"]:
            ...
            low = 1.0 / (low * 2.628e6)  # in months
            high = 1.0 / (high * 2.628e6)
            # get coefficients
            b, a = iirfilter(
                order,
                [low / nyq, high / nyq],
            ...
            raise Exception(
                "For band filter cutoff must be a list of [low,high] for low/high-pass cutoff must be a integer!"
            )
        ...
        self.data = np.squeeze(self.data)
        self.filtered_data = (
            np.squeeze(self.filtered_data)
            if cut is None
            else np.squeeze(self.filtered_data[to_cut:-to_cut, ...])
        )
    ...
    def spatial_filter(self, filter_weights=[1, 2, 1], use_to_data=False):
        ...
        d = np.zeros(
            (
                list(self.data.shape[:-2])
                + [
                    self.lats.shape[0] - len(filter_weights) + 1,
                    self.lons.shape[0] - len(filter_weights) + 1,
                ]
            )
        )
        ...
        for i in range(d.shape[-2]):
            for j in range(d.shape[-1]):
                avg_mask = np.array(
                    [
                        [mask for _ in range(d.shape[1])]
                        for _ in range(d.shape[0])
                    ]
                )
                d[:, :, i, j] = np.average(
                    self.data, axis=(2, 3), weights=avg_mask
                )
        ...
        if use_to_data:
            ...
            if len(filter_weights) % 2 == 1:
                self.lats = self.lats[
                    len(filter_weights) // 2 : -len(filter_weights) // 2 + 1
                ]
                self.lons = self.lons[
                    len(filter_weights) // 2 : -len(filter_weights) // 2 + 1
                ]
    ...
    def _interp_spatial(a):
        ...
        new_data = si.griddata(
            points, d[~msk], (grid_lat, grid_lon), method=method
        )

        return t, new_data

    def interpolate_spatial_nans(
        self, method="cubic", apply_to_data=True, pool=None
    ):
        """
        ...
        if self.nans:
            if self.check_NaNs_only_spatial():
                ...
                raise Exception(
                    "NaNs are also temporal, no way to filter them out!"
                )
        else:
    ...
    def filter_out_NaNs(self, field=None):
        ...
        if (field is None and self.nans) or (
            field is not None and np.any(np.isnan(field))
        ):
    ...
    @staticmethod
    def _rotate_varimax(
        U, rtol=np.finfo(np.float32).eps ** 0.5, gamma=1.0, maxiter=500
    ):
        """
    ...
    def pca_components(self, n_comps, field=None, rotate_varimax=False):
        ...
        if self.data.ndim == 3:
            ...
            # if nans, filter-out
            if (self.nans and field is None) or (
                field is not None and np.any(np.isnan(field))
            ):
            ...
            if rotate_varimax:
                ...
                rot = np.matrix(T)
                S2 = np.dot(
                    np.dot(np.transpose(rot), np.matrix(np.diag(var))), rot
                )
                expvar = np.diag(S2)
                pcs = np.array(
                    np.dot(np.transpose(rot), np.diag(s[:n_comps])) * pcs.T
                )
                # var
            ...
            raise Exception(
                "PCA analysis cannot be used on multi-level data or only temporal (e.g. station) data!"
            )
    ...
    def anomalise(self, base_period=None, ts=None):
        ...
        delta = self.time[1] - self.time[0]
        seasonal_mean = (
            np.zeros_like(self.data) if ts is None else np.zeros_like(ts)
        )
        ...
        if base_period is None:
            ...
            ndx = np.logical_and(
                self.time >= base_period[0].toordinal(),
                self.time <= base_period[1].toordinal(),
            )
        ...
        if delta == 1:
            ...
            for mi in range(1, 13):
                ...
                for di in range(1, 32):
                    ...
                    if np.sum(sel_avg) == 0:
                        continue
                    seasonal_mean[sel_data, ...] = np.nanmean(
                        d[sel_avg, ...], axis=0
                    )
                    if ts is None:
        ...
        if delta == 1:
            ...
            for mi in range(1, 13):
                ...
                for di in range(1, 32):
                    ...
                    self.data[sel_data, ...] -= seasonal_mean[sel_data, ...]
                    seasonal_var[sel_data, ...] = np.nanstd(
                        d[sel_avg, ...], axis=0, ddof=1
                    )
                    if (
                        np.any(seasonal_var[sel_data, ...] == 0.0)
                        and self.verbose
                    ):
                        print(
                            "**WARNING: some zero standard deviations found for date %d.%d"
                            % (di, mi)
                        )
    ...
    def _get_oscillatory_modes(a):
        ...
        if not np.any(np.isnan(data)):
            wave, _, _, _ = wvlt.continous_wavelet(
                data, 1, True, wvlt.morlet, dj=0, s0=s0, j1=0, k0=k0
            )
            ...
            if amp_to_data:
                ...
                fit_x = np.vstack(
                    [reconstruction, np.ones(reconstruction.shape[0])]
                ).T
    ...
    def _get_parametric_phase(a):
        ...
        if not np.any(np.isnan(data)):
            ...
            upper_bound = (
                half_length + 1 if data.shape[0] & 0x1 else half_length
            )
            ...
            z = mx * np.cos(
                np.arange(-half_length, upper_bound, 1) * freq + phi
            )
            ...
            upper_bound_window = (
                half_window + 1 if window & 0x1 else half_window
            )
    ...
    def get_parametric_phase(
        ...
        if delta == 1:
            ...
            if period_unit == "y":
                ...
                raise Exception(
                    "For daily data is hard to enter wavelet period in months..."
                )
            else:
            ...
            if period_unit == "y":
                ...
                raise Exception(
                    "For monthly data doesn't make sense to enter wavelet period in days."
                )
            else:
                raise Exception("Unknown type.")
        ...
        if ts is None:
            ...
            self.phase = (
                ...
                if cut is None
                    ...
                    [self.data.shape[0] - 2 * to_cut] + self.get_spatial_dims()
                )
            )
            if save_wave:
                self.wave = (
                    np.zeros_like(self.data, dtype=np.complex64)
                    if cut is None
                    else np.zeros(
                        [self.data.shape[0] - 2 * to_cut]
                        + self.get_spatial_dims(),
                        dtype=np.complex64,
                    )
                )
            ...
            self.data = np.squeeze(self.data)
            self.phase = np.squeeze(
                self.phase
            )  # if cut is None else np.squeeze(self.phase[to_cut:-to_cut, ...])
            if save_wave:
                self.wave = np.squeeze(
                    self.wave
                )  # if cut is None else np.squeeze(self.wave[to_cut:-to_cut, ...])

        else:
            res = self._get_parametric_phase(
                [
                    0,
                    0,
                    self.frequency,
    ...
    def wavelet(
        ...
        if ts is None:
            ...
            if phase_fluct:
                ph0 = self.phase[0, ...]
                sin = (
                    np.arange(0, self.phase.shape[0])[:, np.newaxis, np.newaxis]
                    * self.omega
                    + ph0
                )
                self.phase -= sin
            ...
            res = self._get_oscillatory_modes(
                [
                    0,
                    0,
                    s0,
    ...
    def quick_render(
        ...
        if self.data.ndim == 3:
            ...
            if mean:
                ...
                title = "%s at %d level: temporal mean" % (
                    self.var_name.upper(),
                    lvl,
                )
        ...
        if field_to_plot is not None and not station_data:
            if (
                field_to_plot.ndim == 2
                and field_to_plot.shape[0] == self.lats.shape[0]
                and field_to_plot.shape[1] == self.lons.shape[0]
            ):
                field = field_to_plot
                title = "Some field you should know"
            else:
                raise Exception(
                    "field_to_plot has to have shape as lats x lons saved in the data class!"
                )

        if station_data:
            if field_to_plot.ndim != 1:
                raise Exception(
                    "Station data must be passed as time x station!"
                )

            import scipy.interpolate as si

            # 0.1 by 0.1 grid
            lats_stations = np.arange(
                self.lats.min(), self.lats.max() + 0.1, 0.1
            )
            lons_stations = np.arange(
                self.lons.min(), self.lons.max() + 0.3, 0.3
            )
            grid_lat, grid_lon = np.meshgrid(
                lats_stations, lons_stations, indexing="ij"
            )  # final grids
            points = np.zeros((self.lons.shape[0], 2))
            points[:, 0] = self.lats
            points[:, 1] = self.lons
            field = si.griddata(
                points, field_to_plot, (grid_lat, grid_lon), method="nearest"
            )
        ...
        if whole_world:
            ...
            if np.any(llons < 0.0):
                ...
                llons.append(360.0)
                lons = np.array(llons)
                end_lon_shift = np.sort(lons - 180.0)
                end_lon_shift = end_lon_shift[end_lon_shift >= 0.0]
                data, lons = shiftgrid(
                    end_lon_shift[0] + 180.0, data, lons, start=False
                )
            else:
                lons = np.array(llons)
                data = field.copy()
            ...
            draw_lats = np.arange(
                np.around(lats[0] / 5, decimals=0) * 5,
                np.around(lats[-1] / 5, decimals=0) * 5,
                10,
            )
            draw_lons = np.arange(
                np.around(lons[0] / 5, decimals=0) * 5,
                np.around(lons[-1] / 5, decimals=0) * 5,
                20,
            )
            m.drawparallels(
                draw_lats,
            ...
            m.drawcoastlines(linewidth=1.0, color="#333333")
        ...
        # draw contours
        cmap = (
            plt.get_cmap(cmap) if cmap is not None else plt.get_cmap("viridis")
        )
        if log is not None:
            levels = np.logspace(
                np.log10(min) / np.log10(log),
                np.log10(max) / np.log10(log),
                levels + 1,
            )
            cs = m.contourf(
                x,
            ...
            if colormesh:
                ...
                cs = m.pcolormesh(
                    x, y, data, vmin=levels[0], vmax=levels[-1], cmap=cmap
                )
            else:
                cs = m.contourf(
                    x, y, data, levels=levels, cmap=cmap, extend=extend
                )
        ...
        cbar.ax.set_yticklabels(
            np.around(levels[::4], decimals=2), size=size_parallels
        )
