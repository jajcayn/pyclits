...
def load_aggregated_data(frame, actual_dataset, actual_metadata):
	...
	for row in frame:
		...
		actual_dataset[date] = [open_price, max_price, min_price, close_price, difference_open_price, difference_max_price, difference_min_price,
		                        difference_close_price, volume]

	actual_metadata[
		'format'] = "open_price, max_price, min_price, close_price, difference_open_price, difference_max_price, difference_min_price, difference_close_price, volume"
	actual_metadata['price_multiplicator'] = price_multiplicator
...
def price_minute_analysis(prefix, dataset, dpi=400, bins=250):
	...
	dataset_dict = {
		"x": (bins_bid[1:], bins_bid[1:]), "y": (cumsum_bid, interp_values), "xscale": "log", "yscale": "log",
		'label': ["val", r"$x^{" + f"{popt[0]:.3f}" + r"}$"], "legend": True
	}
	...
	dataset_dict = {
		(0, 0): {'x': (bins_bid_open[:shape_open + 1] * (-1), bins_bid_open[shape_open:]), 'y': (hist_open[:shape_open + 1], hist_open[shape_open - 1:]),
		         'label': ["-", "+"], 'title': "open", "xscale": "log", "yscale": "log"},
		(1, 0): {'x': (bins_bid_max[:shape_max + 1] * (-1), bins_bid_max[shape_max:]), 'y': (hist_max[:shape_max + 1], hist_max[shape_max - 1:]),
		         'label': ["-", "+"], 'title': "max", "xscale": "log", "yscale": "log"},
		(0, 1): {'x': (bins_bid_min[:shape_min + 1] * (-1), bins_bid_min[shape_min:]), 'y': (hist_min[:shape_min + 1], hist_min[shape_min - 1:]),
		         'label': ["-", "+"], 'title': "min", "xscale": "log", "yscale": "log"},
		(1, 1): {'x': (bins_bid_close[:shape_close + 1] * (-1), bins_bid_close[shape_close:]),
		         'y': (hist_close[:shape_close + 1], hist_close[shape_close - 1:]), 'label': ["-", "+"], 'title': "close", "xscale": "log", "yscale": "log"},
	}
