...
def load_datasets():
    ...
    datafiles = [
        ...
        if "bin" not in file and os.path.isfile(directory / file)
    ]
...
def price_analysis(prefix, dataset, dpi=400, number_bins=250):
    ...
    interp_bin1, interp_values1, popt1 = interpolation(
        bins[1:], hist, selector=lambda x: x < -70, multiplicator=-1
    )
    interp_bin2, interp_values2, popt2 = interpolation(
        bins[1:], hist, selector=lambda x: x > 70, multiplicator=1
    )
    ...
    interp_bin, interp_values, popt = interpolation(
...
def price_minute_analysis(prefix, dataset, dpi=400, bins=250):
    ...
    dataset_dict = {
        (0, 0): {
            "x": (bins_bid_open[: shape_open + 1] * (-1), bins_bid_open[shape_open:]),
