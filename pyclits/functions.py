...
def detrend_with_return(arr, axis=0):
    ...
    newdata = np.reshape(
        np.transpose(a, tuple(newdims)),
        (a.shape[axis], np.prod(a.shape, axis=0) // a.shape[axis]),
    )
...
def get_haar_flucs(
    ts, min_dt=2, run_backwards=True, spacings=[2, 4, 8, 16], rms=True
):
    """
    ...
    if spacings is None:
        dts = np.arange(
            min_dt, max_dt, 2
        )  # only even as we are dividing the interval into two
    else:
        dts = np.concatenate(
            [
                np.arange(
                    i * int(np.ceil(max_dt // len(spacings) / 2) * 2),
                    (i + 1) * int(np.ceil(max_dt // len(spacings) / 2) * 2),
                    sp,
                )
                for sp, i in zip(spacings, range(len(spacings)))
            ]
        )
    ...
    for run in range(runs):
        ...
        for i in range(dts.shape[0]):
            ...
            splitted = splitted.reshape(
                (splitted.shape[0], dts[i] // 2, 2), order="F"
            )
