...
class ssa_class:
    ...
    def run_ssa(self):
        ...
        # eigendecomposition
        _, lam, e = np.linalg.svd(
            self.C, compute_uv=True
        )  # diag(lambda) = E.T * C * E, lambda are eigenvalues, cols of E are eigenvectors
        e = e.T
            ...
            if multivariate:
                ...
                for d in range(self.X.shape[1]):
                    if not residuals:
                        ...
                        self.MCsurrs[i, :, d] = np.squeeze(
                            vs[d].simulate_with_residuals(
                                vs["res" + str(d)], orig_length=True
                            )
                        )
    ...
    def run_Monte_Carlo(
        ...
        if method not in [
            ...
            raise Exception(
                "Method not know. Please use one of the 'data', 'separately', 'ensemble', 'rotation', 'rank-def'."
            )

        # get the ensamble of surrogate data
        self._get_MC_realizations(
            n=n_realizations, multivariate=multivariate, residuals=residuals
        )

        if (self.n - self.M + 1) < self.d * self.M:
            print(
                "**WARNING: with the current SSA settings, the cov. matrix is rank deficient. Keep in mind."
            )

        if method == "rank-def":
            if (self.n - self.M + 1) > self.d * self.M:
                print(
                    "**WARNING: rank deficient method is usually used when N' < DM. Otherwise, 'rotation' method works best."
                )
            print(
                "**WARNING: in rank deficient matrix the problem is more compicated, rely more on visual spectra than computational method."
            )
            # cov matrix
            C_rd, _ = self._get_cov_matrix(self.X, rank_def=True)
            # eigendecomposition
            _, l_rd, e_rd = np.linalg.svd(
                C_rd, compute_uv=True
            )  # diag(lambda) = E.T * C * E, lambda are eigenvalues, cols of E are eigenvectors
            e_rd = e_rd.T
            assert np.allclose(
                C_rd, np.dot(e_rd, np.dot(np.diag(l_rd), e_rd.T))
            )
        ...
        if method == "ensemble":
            C_mean_surr = np.mean(
                np.array(
                    [
                        self._get_cov_matrix(self.MCsurrs[i, ...])[0]
                        for i in range(n_realizations)
                    ]
                ),
                axis=0,
            )
            _, ltemp, e_mean_surr = np.linalg.svd(
                C_mean_surr, compute_uv=True
            )  # diag(lambda) = E.T * C * E, lambda are eigenvalues, cols of E are eigenvectors
            e_mean_surr = e_mean_surr.T
            assert np.allclose(
                C_mean_surr,
                np.dot(e_mean_surr, np.dot(np.diag(ltemp), e_mean_surr.T)),
            )

            eig_data_ensemble = np.diag(
                np.dot(e_mean_surr.T, np.dot(self.C, e_mean_surr))
            )
        ...
        for i in range(n_realizations):
            if method == "rank-def":
                C_surr = self._get_cov_matrix(
                    self.MCsurrs[i, ...], rank_def=True
                )[0]
            else:
            ...
            if method == "data":
                ...
                eig_s = np.diag(
                    np.dot(e_mean_surr.T, np.dot(C_surr, e_mean_surr))
                )
                # normalise
                eig_s = eig_s / np.sum(eig_s)
                # sort
                ...
                if method == "rotation":
                    u, _, v = np.linalg.svd(
                        np.dot((eig ** 0.5) * e_s.T, (self.lam ** 0.5) * self.e)
                    )
                elif method == "rank-def":
                    u, _, v = np.linalg.svd(
                        np.dot((eig ** 0.5) * e_s.T, (l_rd ** 0.5) * e_rd)
                    )
                T_e = np.dot(u, v)
        ...
        if plot:
            ...
            plt.plot(
                np.percentile(eigvals_surrs[:, :40], q=97.5, axis=0),
                marker="^",
                markersize=6,
                linestyle="none",
                color="g",
            ...
            plt.plot(
                np.percentile(eigvals_surrs[:, :40], q=2.5, axis=0),
                marker="v",
                markersize=6,
                linestyle="none",
                color="g",
            ...
            plt.title(
                "DATA EIGENVALUES vs. %d MONTE-CARLO AR(1)" % (n_realizations),
                size=25,
            )
            plt.show()

        prctl = (
            np.percentile(eigvals_surrs, q=1 - p_value / 2.0, axis=0)
            if not self.rotated
            else np.percentile(
                eigvals_surrs[:, : self.S], q=1 - p_value / 2.0, axis=0
            )
        )
        num_sign = 0
    ...
    def run_enhanced_Monte_Carlo(
        ...
        lam_freqs = np.digitize(
            np.abs(
                [
                    freqs[np.fft.fft(self.pc[:, i]).argmax()]
        ...
        for i in range(n_realizations):
            ...
            # sort to bins
            lam_freqs_surrs[i, ...] = np.digitize(
                np.abs(
                    [
                        freqs[np.fft.fft(pc_surr[:, ii]).argmax()]
                        for ii in range(pc_surr.shape[1])
                    ]
                ),
                freq_bins,
            )
        ...
        for ii in range(to_plot):
            plt.plot(
                freq_bins[lam_freqs[ii]] + np.diff(freq_bins)[0] / 2.0,
                self.lam[ii],
                marker="o",
                markersize=10,
                linestyle="none",
